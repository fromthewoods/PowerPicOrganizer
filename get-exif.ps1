    ...
    Process
    {
        Foreach ($id in $exifID)
        {
            ...
            If ($item)
            {
                ...
                {
                    If (($ExifID -eq $ExifIDExposuretime) -and ($item.Value.Numerator -eq 1) )
                    {
                        "1/$($item.Value.Denominator)"
                    }
                    Else
                    {
                        $item.value.value
                ...
                ElseIf (($item.type -eq 1101) -or ($item.type -eq 1100)) # "VectorOfByte"=1101
                {
                    $item.value.string()
                }
                Else
...
#>
    Param
    (
        [Parameter(ValueFromPipeline=$true,Mandatory=$true)]$image
    )
    Process
    {
        ...
        If ($dt) {
            $dtculture = [DateTime]::ParseExact($dt,"yyyy:MM:dd HH:mm:ss",[System.Globalization.CultureInfo]::InvariantCulture)
            # Getting date time in yyyyMMdd_HHmmss format for filesystem organization
            $dtFS = $dt.Replace(":",""); $dtFS = $dtFS.Replace(" ","_")
        }
...
<#
    ...
    .LINK
        http://u88.n24.queensu.ca/exiftool/forum/index.php?topic=4253.20;wap2
#>
    Param
    (    
